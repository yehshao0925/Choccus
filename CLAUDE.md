...
| `npm run meta-rank [-- --map=<m>]` | **cycle-aware 評估**（α-Rank ＋ Nash-averaging）：純讀現成 `bt-history`、**不對打、不寫檔、秒級**。對刻意非遞移（RPS）roster，BT 單一純量會壓縮環、把近 clone 重複計數；此工具印 α-Rank（演化穩態）＋ Nash-averaging（**對 clone 不變**）並列 BT，外加 clone check（回答「v6 池放 v4+v5 近複製品會不會扭曲」）。是 BT＋殘差的正規版交叉檢查（詳見 `docs/ai-versions.md` §七） |
...
| `npm run v5-screen [-- --save-baseline]` | **最快首篩**（paired CRN ＋ sequential early-stop，~30–60 s）：先 `--save-baseline`（flag **off**＝現役冠軍）存逐局 baseline，再翻 flag **on** 跑 `v5-screen` → 同 seed 逐局**配對**相減（消地圖運氣變異 → 小 N 也分得出真效果）＋寬門檻（Z=1.5，容忍雜訊）→ 印 **ESCALATE／DROP／INCONCLUSIVE**。預設對手＝ship-gate `v4:zoner`＋binding mirror `v3:trapper`（gate 顯著變差直接 DROP，擋過擬合）。**只有 ESCALATE 才值得花完整 `bt-rank`**。換冠軍才重存 baseline；`--map` 預設 pirate，`--max-repeats`／`--block`／`--min-repeats` 可調 |
...
- 火力 2（上限 **7**）
- 炮數 1（上限 **6**）
- 速度加成 0（每顆 **+1.0**，上限 **+3.0** → 極速 **8.0 格/秒**）
...
> 2026-06-24 速度對齊 BnB 囡囡（Uni）：速度加成 +0.4/上限+2.0（極速 7.0）→ **+1.0/上限+3.0（極速 8.0）**，＝囡囡 5/8（base 5・滾輪鞋每顆 +1 階・3 顆滿）。彈數 1/6、火力 2/7 本來就＝囡囡，未動。改動改變 sim → **須重 seed BT 量尺兩圖**；若有撿速度道具的 replay 偏移則重 pin golden（詳見 `docs/bnb-reverse-engineering.md` §1.4）。
...
- **開發新策略（v5＋）的快速迴圈＝三階段、由快到慢、只在有訊號才升級**：
  ...
  3. **完整評估**：`bt-rank` 上量尺（全池 Elo＋殘差）＋ `meta-rank`（cycle-aware／clone 交叉檢查）＋ `npm test` ＋ `npm run lint`。
  **出貨判準＝對現役冠軍 `v5:zoner` 兩圖直接 CRN 勝率 ≥ 50%**（光是 v3-池 Elo 高不算數，可能輸給現役 bot；`v4:zoner`／`v5:zoner` 已在量尺池中，v6 直接對前沿量）。caps 若再動須重 `bt-seed` 兩圖。
